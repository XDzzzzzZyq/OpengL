/**
 * @file SceneManager.h
 * @brief Scene resource management and scene graph organization.
 * 
 * Provides SceneResource (container for all scene objects) and SceneManager
 * (factory for preset scenes). SceneResource aggregates cameras, lights, meshes,
 * environments, and other renderable objects into a unified scene graph.
 * 
 * Architecture:
 * - SceneResource: Owns all scene objects via shared_ptr maps
 * - SceneManager: Static factory for creating preset scenes (Main, CornellBox, etc.)
 * - Context queries SceneResource to obtain active camera, environment, object lists
 * - Renderer reads SceneResource as immutable scene graph
 * 
 ...
 * 
 * SceneResource aggregates all renderable and non-renderable objects in a scene.
 ...
 * 
 * Caching Strategy:
 * - Scenes stored in sce_configs map (name -> SceneResource)
...
private: 
	...
	static std::unordered_map<std::string, std::shared_ptr<SceneResource>> sce_configs; ///< Scene cache (name -> resource)
...
public:                        
	 ...
	 * @return Shared pointer to SceneResource
	 ...
	 * @return Shared pointer to SceneResource, or nullptr if not cached
	 */
